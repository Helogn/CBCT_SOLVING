...
Data_path = "D:\MRES\Label\Catch010\";

Scan1_path = '20201005.nii';
Scan2_path = '20201008.nii';
Scan3_path = '20201013.nii';
Scan4_path = '20201016.nii';
...
Scan10(Scan_label == 0) = -1000;



%% compare differences among these five images
Subtract = zeros([3,sz(1),sz(2),sz(3)]);
Subtract(1,:,:,:) = Scan10(1,:,:,:) - Scan10(2,:,:,:);
Subtract(2,:,:,:) = Scan10(1,:,:,:) - Scan10(3,:,:,:);
Subtract(3,:,:,:) = Scan10(1,:,:,:) - Scan10(4,:,:,:);
% Subtract(4,:,:,:) = Scan(1,:,:,:) - Scan(5,:,:,:);
Subtract = abs(Subtract);

%% MIP Label = 1   Background = 0
A = 4;
MIP1 = MIP(squeeze(Scan10(1,:,:,:)),squeeze(Scan_label(1,:,:,:)),A);
MIP2 = MIP(squeeze(Scan10(2,:,:,:)),squeeze(Scan_label(2,:,:,:)),A);
MIP3 = MIP(squeeze(Scan10(3,:,:,:)),squeeze(Scan_label(3,:,:,:)),A);
MIP4 = MIP(squeeze(Scan10(4,:,:,:)),squeeze(Scan_label(4,:,:,:)),A);
