...
Data_path = "D:\MRES\Label\Catch003\";

Scan1_path = '20201027.nii';
Scan2_path = '20201030.nii';
Scan3_path = '20201104.nii';
Scan4_path = '20201109.nii';
...
Scan03(Scan_label == 0) = -1000;



%% compare differences among these five images
Subtract = zeros([3,sz(1),sz(2),sz(3)]);
Subtract(1,:,:,:) = Scan03(1,:,:,:) - Scan03(2,:,:,:);
Subtract(2,:,:,:) = Scan03(1,:,:,:) - Scan03(3,:,:,:);
Subtract(3,:,:,:) = Scan03(1,:,:,:) - Scan03(4,:,:,:);
% Subtract(4,:,:,:) = Scan(1,:,:,:) - Scan(5,:,:,:);
Subtract = abs(Subtract);

%% MIP Label = 1   Background = 0
A = 6;
MIP1 = MIP(squeeze(Scan03(1,:,:,:)),squeeze(Scan_label(1,:,:,:)),A);
MIP2 = MIP(squeeze(Scan03(2,:,:,:)),squeeze(Scan_label(2,:,:,:)),A);
MIP3 = MIP(squeeze(Scan03(3,:,:,:)),squeeze(Scan_label(3,:,:,:)),A);
MIP4 = MIP(squeeze(Scan03(4,:,:,:)),squeeze(Scan_label(4,:,:,:)),A);
