...
with open("Catch" + str(Catch_num)+'.bat', 'w') as f:
    ...
    for k in (Slice):
        ...
        if A == 1:
            ...
            aladin = "reg_aladin.exe -flo D:\\MRES\\Label\\Catch%s\\%s.nii -ref D:\MRES\Label\Catch%s\PCT.nii -res D:\MRES\Label\Catch%s\T%s.nii -aff D:\MRES\Label\Catch%s\\txt\\C%s.txt -rigOnly -ln 4 -lp 3 -%%%%v 100 -%%%%i 60 \n" % (Catch_num,k,Catch_num,Catch_num,k,Catch_num,k)
            f.write(aladin)
